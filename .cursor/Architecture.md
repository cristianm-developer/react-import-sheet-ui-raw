...
- **Step completado:** **Step 5** (Data Phase). Implementados **useRawDataTable** (Roaming Tabindex, editingEnabled, RawDataTableProvider), **useRawTableHead** (headers), **useRawTableBody** (getRowProps, totalRowCount, isPlaceholder), **useRawTableRow**, **useRawCell** (getCellProps, getEditInputProps, getErrorProps, data-pending, aria-invalid), **useRawErrorBadge**; tipos **TableHeader**, **RawTableRowContext**, **RawCellContext**, **GetRowPropsResult/Options**, **GetCellPropsOptions** en `src/shared/types/data-phase.ts`. Siguiente: Step 6 (view phase).
